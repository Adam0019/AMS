                ...
                <tbody>
                   <?php
                    foreach ($result as $row) {
                     ...
                     data-c_name="<?php echo $row['c_name']; ?>"
                     data-gl_name="<?php echo $row['gl_name']; ?>"
                     data-c_id="<?php echo $row['c_id']; ?>"
                     data-gl_id="<?php echo $row['gl_id']; ?>"
                       data-cheque_number="<?php echo isset($row['cheque_number']) ? $row['cheque_number'] : ''; ?>"
                data-bank_name="<?php echo isset($row['bank_name']) ? $row['bank_name'] : ''; ?>"
                data-cheque_date="<?php echo isset($row['cheque_date']) ? $row['cheque_date'] : ''; ?>"
...
try {
    ...
    echo "Error fetching GL accounts: " . $e->getMessage();
    $gls = [];
}
?>
  ...
  <input type="text" class="form-control" id="c_name" name="c_name" placeholder="Enter Customer Name">
            ...
            <div class="form-group mt-4">
                 ...
                 <?php foreach($gls as $gl): ?>
                    <option value="<?php echo $gl['gl_id']; ?>"><?php echo htmlspecialchars($gl['gl_name']); ?></option>
                 <?php endforeach; ?>
                   </select>  
                  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <!-- <div class="view-cheque-details" id="viewChequeDetails" style="display: none;">
      <hr>
      <h6 class="text-muted">Cheque Details</h6> -->
      <div class="row mb-2">
         <div class="col-4"><strong>Cheque Number:</strong></div>
         <div class="col-8"><span name="cheque_number" id="view_cheque_number"></span></div>
      </div>
      <div class="row mb-2">
         <div class="col-4"><strong>Bank Name:</strong></div>
         <div class="col-8"><span name="bank_name" id="view_bank_name"></span></div>
      </div>
      <div class="row mb-2">
         <div class="col-4"><strong>Cheque Date:</strong></div>
         <div class="col-8"><span name="cheque_date" id="view_cheque_date"></span></div>
      </div>
      </div>
      </div>
      <div class="modal-footer">
        ...
        <form id="editCreditForm" method="POST" action="updateCredit.php">
          ...
          <div class="form-group mt-4">
            ...
            <select class="form-control" id="edit_c_id" name="c_id" required>
              <option value="" disabled selected>From</option>
              <?php foreach($customers as $customer): ?>
                <option value="<?php echo $customer['c_id']; ?>"><?php echo htmlspecialchars($customer['c_name']); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
...
// View Credit Modal
  ...
  viewButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      document.getElementById('view_cheque_number').textContent = button.getAttribute('data-cheque_number');
      document.getElementById('view_bank_name').textContent = button.getAttribute('data-bank_name');
      document.getElementById('view_cheque_date').textContent = button.getAttribute('data-cheque_date');
...
document.getElementById('confirmDelete').addEventListener('click', function() {
  ...
  if (creditId) {
    fetch('delete_credit.php', {
      ...
      body: 'credit_id=' + encodeURIComponent(creditId)
    })
    .then(res => res.text())
    .then(response => {
      if(response.includes('Credit deleted successfully')){
